...
export default function Home(){
  ...
  async function load(uid:string){
    ...
    if(b?.[0]){
      ...
      setApartments(a||[]);
      const{data:i}=await s.from("invitations").select("id,apartment_id,email,status,expires_at").eq("building_id",b[0].id).order("created_at",{ascending:false});
      setInvitations(i||[]);
    }
  }
  ...
  return <main className="shell">
    ...
    <div className="card">
      ...
      {apartments.map(a=>{
        const pending=invitations.find(i=>i.apartment_id===a.id&&i.status==="pending"&&new Date(i.expires_at)>new Date());
        return <div className="apt" key={a.id}>
          <div><b>Apartment {a.apartment_number}</b><div className="muted">{pending?`Invitation pending: ${pending.email}`:"No active tenant"}</div></div>
          {pending?<span className="secondary" style={{padding:"8px 10px",borderRadius:10}}>INVITED</span>:<button className="secondary" onClick={()=>setInvite(a)}>Invite Tenant</button>}
        </div>
      })}
    </div>
